...
# ╔═╡ b3e713e0-c805-11ec-1fa8-675f31c448eb
begin
	using ComponentArrays,DifferentialEquations,SparseDiffTools,LinearAlgebra,StaticArrays,ThreadedIterables,BenchmarkTools
...
# ╔═╡ bdea7bfa-037b-4680-b050-3c3e529f0b89
using PlutoUI,ColorSchemes

# ╔═╡ 8ec7ca45-dc74-4499-84d5-fadc74f5cdf5
dy(y,x,χ) = SA[-4χ*x[1], ( -exp(-x[2])*cos(0.5*y[1]) + exp(-2*x[2])*cos(y[1]) )/χ]

# ╔═╡ 06167e4f-8584-4605-80eb-77225effe072
dx(y,x,χ) = SA[ (exp(-x[2])*sin(0.5*y[1]) - exp(-2*x[2])*sin(y[1]) )/(2χ), -χ*y[2]]

# ╔═╡ cd648a2d-b634-4330-acd7-5a0117977da9
H(x,χ) = χ*x[1]^2+(1-exp(-x[2]))^2/(4χ)

# ╔═╡ fbe7fcd4-4971-4ea2-b11d-e0b1cc43db1d
function Z_integrand(node,u,θ,χ)
	exp( u.Δ - θ*H(node,χ) + 0.5*log( abs(det(u.Mx)) )  )*ifelse( u.μ % 2 == 0, 1,0 )
end

# ╔═╡ f410837f-c102-4548-82fb-b51293d162bf
...
# ╔═╡ 51886f58-f8e4-4319-91f3-7649394f92c6
u0(x0) = ComponentArray( y=SA[0.0,0.0],x=x0,My= zeros(2,2),Mx=[1.0 0.0;0.0 1.0],Δ=0.0 ,μ=0)

# ╔═╡ 962d7f7d-8379-4d12-843f-799082c901f2
function condition(u,t,integrator)
  det(u.Mx)
end

# ╔═╡ 36115ad2-2e72-425c-b5a2-b5c95ad634b0
function affect!(integrator)
  integrator.u.μ+=1
end

# ╔═╡ f7bdf74f-3988-4f59-9a1a-9646360b65a8
cb = ContinuousCallback(condition,affect!)

# ╔═╡ 68e1811d-35e7-4ed1-88ec-732c7b2a1611
#@bind θ Slider(0.3:0.1:5,show_value=true,default=2.)
θ=3.

# ╔═╡ 01f927e6-3691-4b71-a067-cd82fa0f3550
χ=0.08

# ╔═╡ b4fdd130-6706-4833-84fb-3880898f2801
begin
	N=100
...
# ╔═╡ 03d0599a-da6e-49b7-9fb6-659a7a8a962c
...
# ╔═╡ ce77a795-8f01-4b57-8e80-a2f295d21359
Zs,dets = get_quantities(θ,χ,xs);

# ╔═╡ 15d4fe75-f255-4463-b734-a5115dd82af5
begin
	h1 = heatmap(qs,ps,Zs,clims=(0,0.5),title="Integrand Part. Func.")
	h2 = heatmap(qs,ps,dets,clims=(-2,2),color=cgrad([:red,:white,:blue]),title="Jacobian Determinant")
	plot(h1,h2,size=(1000,400),plot_title=L"\theta=%$θ",xlabel=L"q",ylabel=L"p",margin = 5Plots.mm)
	#savefig("θ=$θ.png")
end

# ╔═╡ fb196ba0-f8c4-4635-96c9-720fca4526a1
heatmap(qs,ps,Zs,clims=(0,0.5),xlabel=L"q",ylabel=L"p")

# ╔═╡ d8160247-dc4a-4234-9261-86210e9b31a8
prob = ODEProblem( F!, u0(xs[50]), (0.0,0.5*θ), χ)

# ╔═╡ 17bf6ecd-1e80-4796-ad46-e1167d978aa8
sol = solve( prob,Vern6(),verbose=false,reltol=1e-6 )

# ╔═╡ ad00c7de-b6e1-48a2-b6e7-7f79ccceb5d2
xs[150]

# ╔═╡ 0fc8f18e-d436-446e-890b-6e958bdd7ff2
sol.retcode != :Succes

# ╔═╡ 87a4841e-237b-40f7-aa4b-d8026f0ffbcd
det(sol(3.0).Mx)

# ╔═╡ abcd03dc-7522-4973-909b-a8ec2ea06428
function get_sols(θ,χ,xs)
	...
	solve( ensemble_prob,Vern6(),trajectories=length(xs),verbose=false,reltol=1e-6 )
end

# ╔═╡ 028af932-c686-4752-ab81-3d1b5f00be26
sols = get_sols(θ,χ,xs);

# ╔═╡ 54cefa78-f587-4ef4-8247-6e6130c5bb22
det(sols[50](3.0).Mx)

# ╔═╡ 588d8ae0-02a5-4af6-a922-18e7dde2ffcb
function S(sol,χ)
    result=similar(sol.t)
    for k in eachindex(sol.t)
        result[k] = sol[k].Δ-0.5*sol.t[k]*H(first(sol.u).x,χ)
    end
    result
end

# ╔═╡ 62dabcb7-4313-4896-ad6c-f27898b95193
function M(sol)
    result=similar(sol.t)
    for k in eachindex(sol.t)
        result[k] = det(sol[k].Mx)
    end
    result
end

# ╔═╡ aa7216c7-a665-4db7-8d62-8b039d0b816b


# ╔═╡ 728f92ed-3269-4291-be7f-c3b7be794920
Ss = S(sols[150],χ)

# ╔═╡ b17c230c-af61-45ed-8ccb-3a220c194d78
Ms = M(sols[150])

# ╔═╡ 680f69fa-9a1a-4ab4-81bc-0a1d9e72edfd
H(xs[150],χ) ≤ 1/(4χ)

# ╔═╡ bcc49662-5580-40b4-9b2a-ad4ff703293b
begin
	plot(2*sols[150].t,Ms,label="Determinant",legend=:bottomleft,right_margin=12Plots.mm,xlims=(2.0,2.15),yticks=[-1.5e6,-1e6,-.5e6,0,.5e6,1e6,1.5e6],xticks=[2.025,2.05,2.075,2.1,2.125],xlabel=L"\theta",title=L"\chi=%$χ; \ p = -0.06;\ q=-0.67")
	plot!(twinx(),2*sols[150].t,Ss,legend=:bottomright,color=:red,xticks=:none,label="Action",xlims=(2.0,2.15))
end

# ╔═╡ a9f8487a-3ddf-4b5b-aee4-b7c3591f7d06
xs[150]

# ╔═╡ 10917fcd-c287-4cae-83d2-79173f67641d
#savefig("action_determinant.png")
...
ColorSchemes = "~3.17.1"
ComponentArrays = "~0.11.15"
DifferentialEquations = "~7.1.0"
LaTeXStrings = "~1.3.0"
Parameters = "~0.12.3"
Plots = "~1.28.0"
PlutoUI = "~0.7.38"
SparseDiffTools = "~1.21.0"
StaticArrays = "~1.4.4"
ThreadedIterables = "~0.2.1"
"""
...
[[deps.ThreadedIterables]]
git-tree-sha1 = "f5c6ac9a5f20cbd5ad2762f8372bfe980139c483"
uuid = "11d239b0-c0b9-11e8-1935-d5cfa53abb03"
version = "0.2.1"
