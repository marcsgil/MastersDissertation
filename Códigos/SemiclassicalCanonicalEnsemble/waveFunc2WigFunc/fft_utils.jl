...
function evaluate_at_direct_grid(f::Function,grid::DFTGrid;perform_shift=false,use_GPU=false)
    if use_GPU
        ...
        if perform_shift
            ifftshift(tmap(x->f(x),multi_dim_direct_grid(grid)))
        else
            tmap(x->f(x),multi_dim_direct_grid(grid))
        end
    end
end
...
function evaluate_at_reciprocal_grid(f::Function,grid::DFTGrid;perform_shift=false,use_GPU=false)
    if use_GPU
        ...
        if perform_shift
            ifftshift(tmap(x->f(x),multi_dim_reciprocal_grid(grid)))
        else
            tmap(x->f(x),multi_dim_reciprocal_grid(grid))
        end
    end
end