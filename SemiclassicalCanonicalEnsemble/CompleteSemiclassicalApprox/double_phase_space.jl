...
function calculate_expectation(θs::AbstractArray,fy,fx,(nodes,weights),output_func,reduction,par=nothing; remove_caustics=true)
    ...
    if remove_caustics
        ...
        sols = solve( ensemble_prob,BS3(),trajectories=length(nodes),reltol=1e-1,abstol=1e-2,saveat=θs/2 )
    end
